...
Each SST simulation goes through a number of stages, from start-up to exit. These stages are referred to as the *SST Lifecycle*. Component trees (i.e., a component together with its subcomponents and/or component extensions) are able to perform different actions in each stage through the Component APIs. These are the stages as viewed by elements in the simulation.
...
Init and Complete are multi-phase stages. They continue until component trees collectively stop exchanging events. Likewise, the Run stage continues until all [primary components](../../core/component/control/registerAsPrimaryComponent.md) have signaled they are done or until SST runs out of events to execute. 
...
The build stage largely takes place in the SST-Core. SST first reads the simulation input file; this may be python, json, a checkpoint manifest, or any other input type that SST recognizes. The file defines a graph of components that SST will simulate. SST performs the following actions on the graph:
* *Generation* SST processes the input file, generating a temporary *ConfigGraph*. This object represents the graph structure and contains information needed to construct simulation elements (e.g., parameters).
* *Partitioning* The ConfigGraph is partitioned for ranks and/or threads.  This step is skipped for serial simulations and pre-partitioned ("parallel loaded") configurations.
* *Distribution* The ConfigGraph is distributed to each rank in the system. This step is skipped for serial simulations or pre-partitioned ("parallel loaded") configurations.

Each parallel partition then builds its own portion of the graph, invoking component constructors which in turn load any other elements the component needs (such as subcomponents). 
...
* Any [SharedObjects](../../core/sharedobj/overview.md) must be initialized
* [User subcomponents should be loaded](../../core/component/load/loadUserSubComponent.md)
* [Parameter](../../core/params/class.md) objects passed to element constructors are deleted after the constructor returns. Elements should save any information they may need from the parameter object before their constructor ends.
