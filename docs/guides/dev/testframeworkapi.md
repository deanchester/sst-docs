...
* **expected_rc** (type: int) The expected return code from SST. Note that the test framework treats this as a bool value. If 0, the test will pass if SST runs without errors and returns 0. If non-zero, the test will pass if SST exits with a non-zero return code.
...
### `testing_check_is_nightly`

Returns whether the testing framework is configured to run tests in the 'nightly' category.
...
Returns whether the environment variable `INTEL_PIN_DIRECTORY` is set to a valid directory. The SST CI environment uses this environment variable. It must be manually set in other environments.
