...
<Tabs groupId="release-version">
    ...
    <TabItem value="15.1" label="15.1 Release">
    ...
    <TabItem value="16.0" label="16.0 Release" default>
        * **Parallel support:** All simulations, parallel and serial, can be checkpointed. Checkpointed simulations can be restarted with different parallelism than they were checkpointed with.
        * **Serializable types**
            * API available: Component, SubComponent, ComponentExtension, Module, PortModule
            * POD types, `std::string`
            * All C++ standard containers, `std::tuple`, `std::pair`, `std::atomic`, `std::variant`, `std::optional`, `std::shared_ptr`, `std::weak_ptr`, `std::unique_ptr`
            * SST types: `Link`, `TimeConverter`, `Output`, `RNG:Random`, `RNG:RandomDistribution`, `SharedArray`, `SharedMap`, `SharedSet`, `UnitAlgebra`, `Statistic`, `StatisticOutput`
            * SST Handlers: `Clock::Handler`, `Event::Handler`, `Clock::Handler2`, `Event::Handler2`
            * SST Interfaces: `StdMem`, `SimpleNetwork`
        ...
        * **Checkpoint triggers**
            ...
            * Real (wall) time using the `--checkpoint-wall-period` command line option.
            * On a signal by using the `--sigusr1=sst.rt.checkpoint` or `--sigusr2=sst.rt.checkpoint` command line options.
            * From the SST debug console, if enabled using the `--checkpoint-enable` and `--interactive-start` command line options.
        * **Output format**
            * Checkpoints are placed in a directory named `<prefix>` which is "checkpoint" by default. Use `--checkpoint-prefix` to change the prefix. If the directory already exists, SST will create a new one named `<prefix>_<num>` using a unique integer for `<num>`.
              * The checkpoint directory will be placed in SST's output directory. By default, this is the current working directory. It can be modified using `--output-directory`.
            * Within the directory, there is a subdirectory for each checkpoint which is named `<checkpoint_name>`. The default `<checkpoint_name>` is `<prefix>_<id>_<simulated_time>` where `<id>` is an incrementing integer and `<simulated_time>` is the simulated time at checkpoint. `<checkpoint_name>` name can be modified using `--checkpoint-name-format`.
            * Within the checkpoint subdirectory are a number of files.
                ...
                * Globals: A binary file containing global checkpoint data named `<checkpoint_name>_globals.bin`
                * Data files: One per thread/rank and named `<checkpoint_name>_<rank>_<thread>.bin`
                * Graph: A file capturing the simulation graph structure to allow restarting with a new partition named `<checkpoint name>_config_graph.bin`
        * **Restart**
          * Simulation checkpoints can be restarted with new parallelization. Only built-in partitioners are supported. Support for self-partitioning a restart is planned for the future.
            * Exception: Parallel loaded simulations (i.e., using the `--parallel-load` command line flag) can be restarted with the same degree of parallelization but cannot be arbitrarily repartitioned due to the lack of a unified configuration graph.
          * The `--load-checkpoint` flag is not required to restart, simply pass the checkpoint manifest (`*.sstcpt`) to SST on the command line. 
    </TabItem>
...
The following instructions are valid for SST 16.0 and the repository.
...
* Standard types: `std::tuple`, `std::pair`, `std::atomic`, `std::variant`, `std::optional`, `std::shared_ptr`, `std::weak_ptr`, `std::unique_ptr`
* SST types: `Link`, `TimeConverter`, `Output`, `RNG:Random`, `RNG:RandomDistribution`, `SharedArray`, `SharedMap`, `SharedSet`, `UnitAlgebra`, `Statistic`, `StatisticOutput`
* Handlers: `Clock::Handler`, `Event::Handler`, `Clock::Handler2`, `Event::Handler2`
* Pointers to serializable types
...
### Step 5: Document that the element is checkpointable
...
* Additional flexibility when restarting SST including ability to specify a new manual partitioning and modify additional simulation parameters.
