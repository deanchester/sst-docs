...
If no component registers as primary, then the Exit object will not be used for that simulation and simulation termination must be accomplished through some other mechanism (e.g. `--stop-at` flag, or some other Action object).
